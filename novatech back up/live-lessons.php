...
// Fetch upcoming live lessons (assuming live_lessons table exists)
$stmt = $pdo->prepare("SELECT lesson_name, course_name, date, start_time, end_time, link 
                       FROM live_lessons 
                       WHERE date >= CURDATE() 
        ...
        <main class="container mx-auto px-6 py-8">
            <div class="bg-white rounded-xl shadow-lg p-6 dashboard-card">
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-xl font-bold text-navy">Upcoming Live Lessons</h2>
                    <a href="#" class="text-gold hover:underline">View Archive</a>
                </div>
                <?php if (empty($live_lessons)): ?>
                    <p class="text-gray-600">No upcoming live lessons at the moment.</p>
                <?php else: ?>
                    <div class="space-y-4">
                        <?php foreach ($live_lessons as $lesson): ?>
                            <div class="p-3 border border-gray-200 rounded-lg">
                                <div class="flex justify-between items-start mb-2">
                                    <h3 class="font-medium text-navy"><?php echo htmlspecialchars($lesson['lesson_name']); ?> (<?php echo htmlspecialchars($lesson['course_name']); ?>)</h3>
                                ...
                                <a href="<?php echo htmlspecialchars($lesson['link']); ?>" class="text-gold text-sm font-medium" target="_blank"><?php echo $lesson['day'] == 'Today' ? 'Join Now' : 'Add to Calendar'; ?></a>
                            </div>
                        <?php endforeach; ?>
                    </div>
                <?php endif; ?>
